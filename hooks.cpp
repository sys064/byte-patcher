...
{
    MessageBoxA(NULL, lpString, "ATOM ADDED - GELION HOOK", MB_OK);
...
)
{
    ...
    {
        int result = MessageBoxW(NULL, lpCommandLine, L"Allow Command? - Gelion-Hook", MB_YESNO | MB_ICONQUESTION);
        ...
        if (result == IDYES) {
            ...
            MessageBoxW(NULL, L"Command Executed.", L"Gelion-Hook", MB_OK | MB_ICONINFORMATION);
        }
        else {
...
const wchar_t* droppedFilePath = L"C:\\seemo-deltrix.exe";
...
{
    MessageBoxW(NULL, L"CreateProcessW hooked!", L"GELION HOOK", MB_OK | MB_ICONINFORMATION);
    ...
    MessageBoxW(NULL, messageBoxText.c_str(), L"GELION HOOK", MB_OK | MB_ICONINFORMATION);
...
{
    MessageBoxA(NULL, "exit call bypassed", "Gelion hook", MB_OK);
}
...
void* MyCurlEasyInit()
{
    MessageBoxA(NULL, "curl hooked", "Gelion-hook", MB_OK);
    return FALSE;
}
...
PIMAGE_NT_HEADERS64 DetourGetNtHeaders(void* image_base) {
    ...
    std::ofstream outfile("Gelion_dump.sys", std::ios::binary);
    if (outfile.is_open()) {
    ...
    if (GetCurrentProcessId() == hooks->g_victim_pid)
    {
        ...
        if (dotPos != std::wstring::npos)
        {
            ...
            if (extension == L"sys" || extension == L"exe" || extension == L"dll")
            {
                ...
                MessageBoxW(NULL, lpFileName, L"File Dropping Detected, Grabbing file. - Gelion hook", MB_OK);
...
HMODULE WINAPI HookedLoadLibraryA(LPCSTR lpLibFileName) {
    ...
    if (processID) {
        ...
        std::ofstream outputFile("Gelion_Dumped.dll", std::ios::binary);
...
HMODULE WINAPI HookedLoadLibraryW(LPCWSTR lpLibFileName) {
    ...
    if (processID) {
        ...
        std::ofstream outputFile(L"Gelion_Dumped.dll", std::ios::binary);
...
void hooking::hookdebuggercheck()
{
    ...
    MessageBoxA(NULL, "Hooked Debugger Checks", "Gelion-hook", MB_OK);
    DetourAttach(&(PVOID&)OriginalDebuggerPresent, DetouredIsDebuggerPresent);
    DetourTransactionCommit();
    MessageBoxA(NULL, "Hooked Applied, ANTI ANTI DEBUGGER ENABLED", "Gelion-hook", MB_OK);
}
...
void hooking::HookBSOD()
{
    ...
    MessageBoxA(NULL, "Hooked RtlAdjustPrivilege", "Gelion-hook", MB_OK);
    ...
    MessageBoxA(NULL, "Hooked NtSetInformationProcess", "Gelion-hook", MB_OK);
    DetourTransactionCommit();
    MessageBoxA(NULL, "Hooked Applied, ANTI BSOD ENABLED", "Gelion-hook", MB_OK);
}
...
void hooking::hookfind()
{
    DetourTransactionBegin();
    DetourUpdateThread(GetCurrentThread());
    MessageBoxA(NULL, "Hooking Protection", "Gelion-hook", MB_OK);
    ...
    MessageBoxA(NULL, "Hooked Applied, Disabled all PC Searching", "Gelion-hook", MB_OK);
    DetourTransactionCommit();
}
...
void hooking::dump_drivers()
{
    DetourTransactionBegin();
    DetourUpdateThread(GetCurrentThread());
    OriginalGetNtHeaders = reinterpret_cast<GetNtHeadersFn>(GetProcAddress(GetModuleHandle("Gelion_victim.exe"), "GetNtHeaders"));
...
bool hooking::register_hook(void* func_addr, void* hooked_func_addr, void** original_func_address)
{
    if (MH_CreateHook(func_addr, hooked_func_addr, original_func_address) == MH_STATUS::MH_OK && MH_EnableHook(func_addr) == MH_STATUS::MH_OK) {
        this->hooked_functions.emplace_back(func_addr);
        return true;
    }
}

bool hooking::register_hook(std::uintptr_t func_addr, void* hooked_func_addr, void** original_func_address)
{
    if (MH_CreateHook((void*)(func_addr), hooked_func_addr, original_func_address) == MH_STATUS::MH_OK && MH_EnableHook((void*)(func_addr)) == MH_STATUS::MH_OK) {
        this->hooked_functions.emplace_back((void*)(func_addr));
        return true;
    }
}

void hooking::destroy_hooks()
{
    for (const auto& hook : this->hooked_functions)
    {
        MH_DisableHook(hook);
    }
...
std::shared_ptr<hooking> hooker()
{
    static auto instance = std::make_shared<hooking>();
    return instance;
}


#define BASE std::uintptr_t( GetModuleHandleW( nullptr ) )
void* original_parse;



void hk_parse(nlohmann::json* json_obj, nlohmann::json* a2, nlohmann::json* a3)
{
    try
    {
        static_cast<void (*)(nlohmann::json*, nlohmann::json*, nlohmann::json*)>(original_parse)(json_obj, a2, a3);

        static std::string old_nonce = "";

        if (json_obj)
        {
            const std::string message = json_obj->contains(("message")) ? json_obj->at(("message")) : "";

            if (json_obj->contains(("appinfo")))
            {
                const auto old_session_id = json_obj->at((("sessionid")));
                old_nonce = json_obj->at((("nonce")));
            }

            // can also be a file download
            else if (!json_obj->at(("success")) && message == ("Session is not validated"))
            {
                json_obj->clear();
                json_obj->emplace(("nonce"), old_nonce);
                json_obj->emplace(("message"), ("Session is validated."));
                json_obj->emplace(("success"), true);
            }
            else if (!json_obj->at(("success")) && (message.find(("Username")) != std::string::npos || message.find(("Key")) != std::string::npos || message.find(("license")) != std::string::npos))
            {
                json_obj->clear();
                json_obj->emplace(("info"), nlohmann::json::object({ { ("createdate"), ("9975534554") }, { ("hwid"), ("Cracked") }, {("ip"), ("127.0.0.1") }, { ("lastlogin"), ("9975532000") }, { ("subscriptions"), nlohmann::json::array({ { { ("expiry"), ("9975534554") }, { ("key"), ("Cracked") }, { ("subscription"), ("default") }, { ("timeleft"), 1339100000 } } }) }, { ("username"), ("1N4KVD-ZWLR8Y") } }));
                json_obj->emplace(("message"), ("Logged in!"));
                json_obj->emplace(("success"), true);

                MessageBoxA(nullptr, ("bypassed by Gelion (if you paid you got scammed)"), ("Gelion"), MB_OK);
            }
        }
...
using DownloadFunction = std::vector<unsigned char>(KeyAuth::api::*)(std::string);

// Original function pointer
DownloadFunction original_download = nullptr;

// Hooked function
std::vector<unsigned char> hk_download(KeyAuth::api* instance, std::string fileid) {
    MessageBoxA(NULL, "Keyauth Download Hooked", "Gelion", MB_OK);

    // Call the original function
    return (instance->*original_download)(fileid);
}
void hooking::hook_keyauth_download()
{
    std::uintptr_t download_function_address = scanner()->find_pattern(("E8 ? ? ? ? 4C 8D 9C")).get();

    std::wstring message = L"Found vuln at: " + std::to_wstring(download_function_address);

    MessageBoxW(NULL, message.c_str(), L"Address Found", MB_OK);

    // Initialize MinHook
    MH_Initialize();

    // Create the hook
    MH_CreateHook((void*)download_function_address, hk_download, reinterpret_cast<void**>(&original_download));

    // Enable the hook
    MH_EnableHook((void*)download_function_address);
}


void hooking::bypass_keyauth(int i)
{

    MessageBoxA(NULL, "Loading Gelion keyauth bypass", "Gelion", MB_OK);
    std::this_thread::sleep_for(std::chrono::milliseconds(2000));

    if (i == 1)
    {
        std::uintptr_t nlohman_parse = scanner()->find_pattern(("48 89 5C 24 20 4C 89 44 24 18 48")).get(); // Universal 48 89 5C 24 20 4C 89 44 24 18 48 89 4C 24 08

        std::wstring message = L"Found vuln at: " + std::to_wstring(nlohman_parse);
        MessageBoxW(NULL, message.c_str(), L"Address Found", MB_OK);

        MH_Initialize();
        MH_CreateHook((void*)nlohman_parse, hk_parse, (void**)&original_parse);
        MH_EnableHook((void*)nlohman_parse);
    }
}